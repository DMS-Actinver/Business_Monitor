document.addEventListener('DOMContentLoaded', () => {
    ...
    if (sucursalList) {
        ...
        const sedesData = [
            { n: "Montes Urales (Corp)", lat: 19.427018, lng: -99.204594 },
...
// --- LÓGICA PARA SINERGIA (MATRIZ DE IMPACTO) ---
const ctxSinergia = document.getElementById('graficaSinergia');
if (ctxSinergia) {
    new Chart(ctxSinergia, {
        type: 'bubble',
        data: {
            datasets: [{
                label: 'Montes Urales (Líder)',
                data: [{ x: 45.2, y: 71, r: 25 }],
                backgroundColor: 'rgba(0, 229, 255, 0.6)',
                borderColor: '#00e5ff',
            }, {
                label: 'Monterrey (Riesgo)',
                data: [{ x: 28.5, y: 50, r: 18 }],
                backgroundColor: 'rgba(255, 193, 7, 0.6)',
                borderColor: '#ffc107',
            }, {
                label: 'Guadalajara (Estable)',
                data: [{ x: 35.0, y: 62, r: 20 }],
                backgroundColor: 'rgba(0, 255, 204, 0.6)',
                borderColor: '#00ffcc',
            }, {
                label: 'Querétaro (Crítico)',
                data: [{ x: 12.0, y: 45, r: 12 }],
        ...
        options: {
            ...
            scales: {
                y: {
                    beginAtZero: false,
                    min: 30,
                    title: { display: true, text: '% Eficiencia de Conversión', color: '#fff' },
                    grid: { color: 'rgba(255,255,255,0.05)' },
                    ticks: { color: '#fff' }
                },
                x: {
                    title: { display: true, text: 'Captación Banco (Millones $)', color: '#fff' },
                    grid: { color: 'rgba(255,255,255,0.05)' },
                    ticks: { color: '#fff' }
                }
            },
                ...
                tooltip: {
                    callbacks: {
                        label: function(context) {
                            return context.dataset.label + ': ' + context.raw.y + '% Eficiencia';
                        }
                    }
                }
            }
...
// ==========================================================================
// MÓDULO INTERACTIVO DE CONTROL: SINERGIA DE GRUPO (CORREGIDO)
// ==========================================================================

const bancoRespuestasSinergia = {
    1: "<strong>Análisis Estratégico (Querétaro):</strong> La baja conversión (45%) se debe a que los ejecutivos locales concentraron sus metas en la apertura de cuentas corrientes básicas. <br><br><strong>Acción Prescriptiva:</strong> Presiona <em>'Intervenir'</em> en la tabla para desplegar la asignación automática de portafolios sugeridos a los asesores de esa plaza.",
    2: "<strong>Análisis de Éxito (Montes Urales):</strong> Esta plaza alcanzó el 71% de eficiencia debido a la automatización del barrido de saldos ociosos de cuentas empresariales hacia el Fondo Digital Actinver.<br><br><strong>Acción Recomendada:</strong> Haz clic en <em>'Replicar Modelo'</em> para documentar este flujo lógico y enviarlo al resto de la república.",
    3: "<strong>Análisis de Alerta (Monterrey):</strong> A pesar de tener una gran captación base de $28.5M, la conversión está estancada en la mitad justa (50%). Hay un cuello de botella en la entrega de contratos de inversión firmados.<br><br><strong>Acción de Choque:</strong> Presiona <em>'Alertar Plaza'</em> para congelar metas laxas y forzar revisión de mesas de control.",
    4: "<strong>Simulador de Escenarios (Guadalajara):</strong> Si incrementamos la tasa al 67% (+5%), lograríamos capturar <strong>$1.75 Millones adicionales</strong> que actualmente están rindiendo tasa cero en cuentas de depósito transaccionales.<br><br>El nodo de Guadalajara se desplazaría hacia arriba en tu matriz de impacto visual.",
    5: "<strong>Propuesta Comercial IA:</strong> Recomiendo activar la campaña <strong>'Nómina Inversionista'</strong>. Consiste en parametrizar un gatillo automático para que cada saldo mensual remanente mayor a $10,000 MXN se mueva a fondos de liquidez diaria de forma segura."
};

document.addEventListener('DOMContentLoaded', () => {
    
    // --- ESCUCHA PARA LOS BOTONES DE PREGUNTAS SUGERIDAS ---
    const botonesPregunta = document.querySelectorAll('.btn-pregunta-ia');
    botonesPregunta.forEach(boton => {
        boton.addEventListener('click', function() {
            const id = this.getAttribute('data-id');
            const cuadroText = document.getElementById('boxRespuestaIA');
            if (cuadroText && bancoRespuestasSinergia[id]) {
                cuadroText.innerHTML = "<strong>IA buscando correlaciones...</strong>";
                setTimeout(() => {
                    cuadroText.innerHTML = bancoRespuestasSinergia[id];
                }, 300);
            }
        });
    });

    // --- ESCUCHA PARA LOS BOTONES OPERATIVOS DE LA TABLA ---
    const botonesAccion = document.querySelectorAll('.btn-accion-sinergia');
    botonesAccion.forEach(boton => {
        boton.addEventListener('click', function() {
            const sucursal = this.getAttribute('data-sucursal');
            const tituloAccion = this.getAttribute('data-accion');
            const bitacora = this.getAttribute('data-bitacora');
            
            alert(`SISTEMA OPERATIVO ACTINVER\n------------------------\nSucursal: ${sucursal}\nAcción: ${tituloAccion}\n\nEstatus: Ejecutado con éxito.`);
            
            const cuadroText = document.getElementById('boxRespuestaIA');
            if (cuadroText) {
                cuadroText.innerHTML = `<strong>Acción Confirmada en Bitácora:</strong><br>${bitacora}<br><br><span class="text-success">● Estado registrado bajo la cuenta de Tania Barrios.</span>`;
            }
            
            const badge = document.getElementById('alertaBadge');
            if(sucursal === 'Querétaro' && badge) {
                badge.className = "badge rounded-pill bg-success";
                badge.innerText = "0 ALERTAS PENDIENTES";
            }
        });
    });

    // --- ENTRADA DE TEXTO LIBRE ---
    const inputLibre = document.getElementById('inputLibreIA');
    if (inputLibre) {
        inputLibre.addEventListener('keypress', function(e) {
            if (e.key === 'Enter') {
                e.preventDefault();
                ...
                if (texto !== "") {
                    const cuadroText = document.getElementById('boxRespuestaIA');
                    if (cuadroText) {
                        cuadroText.innerHTML = "<strong>Procesando entrada de texto libre...</strong>";
                        setTimeout(() => {
                            cuadroText.innerHTML = `<strong>Análisis de términos para: "${texto}"</strong><br>No se detectaron desviaciones adicionales fuera del rango de contingencia de las sucursales listadas en el tablero general.`;
                        }, 400);
                    }
                    this.value = "";
                }
    ...
    // --- BOTONES EXTRA DE ENTRADA (IMAGEN/VOZ) ---
    const btnImg = document.getElementById('btnSinergiaImg');
    if (btnImg) {
        btnImg.addEventListener('click', () => alert('Simulador: Procesando matriz mediante reconocimiento visual de patrones.'));
    }

    const btnVoz = document.getElementById('btnSinergiaVoz');
    if (btnVoz) {
        btnVoz.addEventListener('click', () => alert('Simulador: Micrófono abierto. Dicta tu instrucción comercial.'));
    }
});